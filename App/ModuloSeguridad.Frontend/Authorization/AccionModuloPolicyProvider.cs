...
{
    internal class AccionModuloPolicyProvider : IAuthorizationPolicyProvider
    {
        const string POLICY_PREFIX = "AccionModulo";
        public DefaultAuthorizationPolicyProvider FallbackPolicyProvider { get; }

        public AccionModuloPolicyProvider(IOptions<AuthorizationOptions> options)
        {
        ...
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            if (policyName.StartsWith(POLICY_PREFIX, StringComparison.OrdinalIgnoreCase))
            {
                ...
                var accionModulo = policyName[POLICY_PREFIX.Length..].Split("_");
                policy.AddRequirements(new AccionModuloRequirement(accionModulo[0], accionModulo[1]));
